...
define('FLESPAKKET_URL', 'https://www.flespakket.nl/');
...
if (isset($_GET['action'])) {
    ...
    if (isset($_SESSION['FLESPAKKET_VISIBLE_CONSIGNMENTS'])
        ...
        $visibleConsignments = str_replace('|', ', ', trim($_SESSION['FLESPAKKET_VISIBLE_CONSIGNMENTS'], '|'));

        $sql = "SELECT *
                FROM `" . _DB_PREFIX_ . "flespakket`
        ...
        if (count($consignments) > 0) {
            $statusFile = file(FLESPAKKET_URL . 'status/tnt/' . implode('|', $consignments));
    ...
    if ($_GET['action'] == 'post' && is_numeric($_GET['order_id'])) {
        ...
        $return_url = (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $conf_url . $_SERVER['PHP_SELF'] . '?action=' . $flespakket_action . '&order_id=' . $_GET['order_id'] . '&timestamp=' . $_GET['timestamp'];
...
?>
		...
		<body onload="document.getElementById('flespakket-create-consignment').submit();">
            ...
            <form
                action="<?php echo FLESPAKKET_URL . 'plugin/' . $flespakket_plugin_action . $_GET['order_id']; ?>/?return_url=<?php echo htmlspecialchars(urlencode($return_url)); ?>"
                method="post"
...
?>
		...
		<body onload="updateParentWindow();">
            <h4>Consignment <?php echo $consignment_id; ?> aangemaakt [<a href="<?php echo FLESPAKKET_URL; ?>plugin/label/<?php echo $consignment_id; ?>">label bekijken</a>]</h4>
            ...
            <script type="text/javascript">
                function updateParentWindow() {
                    if (!window.opener || !window.opener.Flespakket || !window.opener.Flespakket.PrestashopPlugin) {
...
?>
		...
		<body onload="document.getElementById('flespakket-create-pdf').submit();">
            <h4>Sending data to Flespakket ...</h4>
            <form
                action="<?php echo FLESPAKKET_URL; ?>plugin/genereer-pdf"
                method="post"
...
<?php
    ...
    if ($_GET['action'] == 'process')
    {
        ...
        $packageList = (strpos($_GET['package_list'], '|') !== false) ?
            explode('|', $_GET['package_list']) : array($_GET['package_list']);
...
?>
		...
		<body onload="document.getElementById('flespakket-create-consignmentbatch').submit();">
            <h4>Sending data to Flespakket ...</h4>
            <form
                action="<?php echo FLESPAKKET_URL . 'plugin/verzending-batch'; ?>/?return_url=<?php echo htmlspecialchars(urlencode($return_url)); ?>"
                method="post"
